# -*- coding: utf-8 -*-
# 20180415
# 3的是保存参数，4是保存图和参数，目前尚未成功
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function

from Autoencoder import SAEnoplhV2, SAEnoplhV3, SAEnoplhdevice
...
log_dir = os.path.join(os.getcwd(), 'log_dir_gpu4/')
...
# tf.summary.scalar(name="learning rate", tensor=sae.lr)
# saver = tf.train.Saver(tf.global_variables())
# merged = tf.summary.merge_all()  # 将所有需要保存的数据进行合并
init = tf.global_variables_initializer()
checkpoint_path = os.path.join(log_dir, 'model.ckpt')
builder = tf.saved_model.builder.SavedModelBuilder(log_dir)
with tf.Session(graph=tf.Graph()) as sess:
    # https://github.com/tensorflow/tensorflow/tree/r1.7/tensorflow/python/saved_model
    ...
    # 否则进行模型重新训练
        ...
        for epoch in range(config.training_epochs):
            ...
            # summary_writer.add_summary(summary_str, sae.current_step)  # 将获得的数据加入到summary_writer写到tensorboard
