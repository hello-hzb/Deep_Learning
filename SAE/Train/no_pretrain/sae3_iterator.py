...
from Autoencoder import SAEnoplh, SAEnoplhdevice, SAEnoplhV4, SAEnoplhV2
...
# solution 4 batch_size=100, time=0.17s time_gpu ：0.19~0.2 0.20居多   time_cpu:0.38~0.45 0.0.45居多
# Creates a `Dataset` whose elements are slices of the given tensors
dataset = tf.data.Dataset.from_tensor_slices(plane_image)
dataset = dataset.repeat(config.training_epochs)  #
dataset = dataset.batch(config.batch_size)
dataset = dataset.prefetch(buffer_size=400)
iterator = dataset.make_initializable_iterator()
image = iterator.get_next()
# cpu:44.32       gpu:19.11   gpuv2:14.53   cpuv2 55.73  prefetch 18.57  prefetch v2 14.29 13.37
use_gpu = 1
if use_gpu:
    print ("Using GPU")
