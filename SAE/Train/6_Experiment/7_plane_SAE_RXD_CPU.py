...
# SAE算法和RX算法的检测结果对比，检查图像为圣地亚哥机场图片
...
log_dir = os.path.join(os.getcwd(), '7_plane_SAE_RXD/')
...
with tf.Session() as sess:
    ...
    # # reconstruction = np.reshape(reconstruction, [100, 100, 126])
    #
    # plt.imshow(trX[:, :, 10])
    # plt.show()
    #
    print("Show reconstruct image")
    reconstruction = np.reshape(reconstruction, [100, 100, 126])
    plt.imshow(reconstruction[:, :, 10])
    plt.show()
    #
    # # recon_err_tmp = np.sum(recon_err_tmp, axis=1)
    # ############################
    # print("Show reconstruct image")
    # recon_err = np.reshape(recon_err_tmp, [100, 100])
    # recon_err = (recon_err > 0.0009)
    # plt.imshow(recon_err)
    # plt.show()
    # ###################

    # 计算SAE检测圣地亚哥机场图像的AUC值
    ...
    # 计算RXD检测圣地亚哥机场图像的AUC值
    rxd_score = sio.loadmat("../7_matlab_work/plane_score.mat")
    rxd_score = rxd_score["plane_score"]
    ...
    plt.figure("Plane")
    lw = 2
    plt.plot(fpr, tpr, color='darkorange', lw=lw, label=U'SAE ROC (AUC = %0.4f)' % roc_auc)
