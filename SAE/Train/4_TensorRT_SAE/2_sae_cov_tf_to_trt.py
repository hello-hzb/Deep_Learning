...
# 在实现C++版本的tensorRT时发现结果不正确，主要原因是输入数据需要转置一下
...
log_dir = os.path.join(os.getcwd(), '2_sae_conv/')
path = os.path.dirname(os.path.realpath(__file__))

OUTPUT_NAMES = "conv2d_4/LeakyRelu/Maximum"
UFF_OUTPUT_FILENAME = path + "/2_sae_cov.uff"
...
with tf.Graph().as_default():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        time1 = time.time()
        output = sess.run(cnn_model.conv_out, feed_dict={image: trX})
        output = np.reshape(output, [126, 100, 100])
...
# trt.utils.write_engine_to_file(engine_file, engine.serialize())   # 生成engine文件
# recon_err = sess.run(cnn_model.recon_err, feed_dict={image: trX})  # 需要reshape后列向求和
print("TensorRT inference time is ", time.time()-trt_time)
trt_out = np.transpose(np.reshape(out, (config.input_size, -1)), (1, 0))
trt_recon_err = np.sum(np.square(trt_out-plane_image), axis=1)
roc_auc1 = roc_auc_score(load_label, trt_recon_err)
print("TensorRT AUC is ", roc_auc1)
